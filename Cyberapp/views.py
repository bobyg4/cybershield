...
def Registration(request):
    ...
    if(request.POST):
        ...
        if(password==cpswd):
            ...
            s="insert into registernow (`firstname`,`lastname`,`email`,`password`,`status`) values('"+firstname+"','"+lastname+"','"+email+"','"+password+"',0)"
            c.execute(s)
            con.commit()
...
def login(request):
    if request.method == "POST":
        ...
        user_query = "SELECT COUNT(*) FROM registernow WHERE email=%s AND password=%s AND status=1"
        c.execute(user_query, (email, password))
        user_data = c.fetchall()
        
        # Check if the user is an admin
        admin_query = "SELECT COUNT(*) FROM admin_table WHERE admin_email=%s AND admin_password=%s"
        c.execute(admin_query, (email, password))
        ...
        if user_data[0][0] == 1:
            ...
            messages.error(request, "Invalid email/password combination. Please try again.")
            return render(request, "login.html")
    
    return render(request, "login.html")

##########################Adminpage
def head3_tail3(request):
    return render(request,"head3_tail3.html")
def AdminHome(request):
    return render(request,"AdminHome.html")
def Viewuser(request):
    s="select * from `registernow` order by reg_id desc"
    c.execute(s)
    data=c.fetchall()
    return render(request,"Viewuser.html",{"data":data})
def Approve(request):
    id=request.GET["id"]
    s="update `registernow` set status=1 where reg_id='"+str(id)+"'"
    c.execute(s)
    con.commit()
...
def file_inbox(request):
    ...
    s="select f.filename,f.sender,f.date,f.filetype,f.sending_purpose,a.attack_types,a.rank,f.file_id from attack_table a join filedata_table f on(a.file_id=f.file_id)where f.receiver ='"+email+"' order by date desc"
    # s="select * from reply_table where `to`='"+str(email)+"'"
    c.execute(s)
    data=c.fetchall()
...
def file_outbox(request):
    ...
    s="select f.filename,f.sender,f.date,f.filetype,f.sending_purpose,a.attack_types,a.rank,f.file_id from attack_table a join filedata_table f on(a.file_id=f.file_id)where f.sender ='"+email+"' order by date desc"
    # s="select * from reply_table where `to`='"+str(email)+"'"
    c.execute(s)
    data=c.fetchall()
    ...
    if(data):
        ...
        return render(request,"file_outbox.html",{"data":data})
    else:
...
def feedback(request):
    ...
    if(request.POST):
        ...
        s="insert into feedback_table(`Name_of_user`,`Email`,`Experience_with_app`,`Rating`,`Comments`) values('"+str(Name_of_user)+"','"+str(Email)+"','"+str(Experience_with_app)+"','"+str(Rating)+"','"+str(Comments)+"')"
        c.execute(s)
        con.commit()
        print(s)
...
def viewfeedback(request):
    r="select * from feedback_table order by fid desc"
    c.execute(r)
    data=c.fetchall()
...
def keylogger(request):
    ...
    return render(request,"keylogger.html")

def exploit(request):

    return render(request,"exploit.html")

def adware(request):

    return render(request,"adware.html")

def infected_usb(request):

    return render(request,"infected_usb.html")

def ransomeware(request):

    return render(request,"ransomeware.html")
...
def replyFeedback(request):
    ...
    if request.POST:
        ...
        s="update feedback_table set reply='"+str(reply)+"' where fid='"+str(fid)+"'"
        c.execute(s)
        con.commit()
        return HttpResponseRedirect("/Viewfeedback")
        
    return render(request,"adminReply.html")

def userviewFeedback(request):
    email=request.session.get("uname")
    r="select * from feedback_table where Email='"+str(email)+"'"
    c.execute(r)
    data=c.fetchall()
    return render(request,"userviewFeedback.html",{"data":data})
